...
struct UIState {
  ...
  UIState(Arguments& args, //
    ...
    this->selection_position = this->args.end ? (int)this->tokens.size() - 1 : 0;
    this->tenacious_single_select_indicator = 0;
    this->loop();
  }

  // ncurses
  static constexpr int PAIR_SELECTED = 1;
  nc::window prompt_window = 0;
  nc::window selection_window = 0;
...
again:
    ...
    if (args.prompt) {
      prompt_lines = str::create_prompt_lines(args.prompt, num_columns - 2);
    }
    ...
    if (first_time) {
      selection_window = nc::newwin(selection_rows, num_columns, selection_window_y, 0);
      ...
      if (args.prompt) {
        prompt_window = nc::newwin(prompt_rows, num_columns, prompt_window_y, 0);
      }
    } else {
      nc::wresize(selection_window, selection_rows, num_columns);
      nc::mvwin(selection_window, selection_window_y, 0);
      if (prompt_window) {
        nc::wresize(prompt_window, prompt_rows, num_columns);
        nc::mvwin(prompt_window, prompt_window_y, 0);
      }
    }
  ...
  void handle_confirmation_input() {
    ...
    for (const auto& s : selections) {
      const pipeline::SimplePacket& token = tokens[s];
      ...
      if (current_row >= 0 && current_row < (int)tokens.size()) {
        ...
        while (pos < end) {
          ...
          if (char_is_invalid) {
            ...
            escape_sequence = str::get_escape_sequence(ch[0]);
          }
        ...
        if (invisible_only) {
          const pipeline::SimplePacket& token = tokens[y + scroll_position];
          wattron(selection_window.get(), A_DIM);
