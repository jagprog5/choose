...
struct choose_output {
  ...
  bool operator==(const choose_output& other) const {
    ...
    if (const std::vector<char>* first_intermediate = std::get_if<std::vector<char>>(&o)) {
      ...
      return first.initial_selection_position == second.initial_selection_position &&                                                                                          //
...
std::ostream& operator<<(std::ostream& os, const choose_output& out) {
  if (const std::vector<char>* out_str = std::get_if<std::vector<char>>(&out.o)) {
    ...
    os << "\n";
    if (out_tokens.initial_selection_position.has_value()) {
      os << "(cursor:" << *out_tokens.initial_selection_position << ") ";
    }
    os << "tokens: ";
...
BOOST_AUTO_TEST_CASE(tui_cursor_selection) {
  ...
  choose_output correct_output{CreateTokensResult{std::vector<choose::Token>{"1", "2", "3"}, 1}};
  BOOST_REQUIRE_EQUAL(out, correct_output);
}
